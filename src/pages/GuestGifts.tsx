        ...
        {gifts.length === 0 ? (
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {gifts.map((gift) => (
              ...
              >
                {gift.image_url && (
                  <img src={gift.image_url} alt={gift.name} className="h-36 w-full object-cover" />
                )}
                <div className="flex flex-1 flex-col p-4">
                  <p className="font-medium text-[#3f3450]">{gift.name}</p>
                  {gift.description && (
                    <p className="mt-1 text-sm text-[#8b7a9c]">{gift.description}</p>
                  )}
                  <p className="mt-2 text-sm text-[#6b5d80]">
                    Valor sugerido: {currency.format(gift.suggested_amount)}
                  </p>

                  {gift.claimed_by_me ? (
                    <span className="mt-4 inline-flex w-fit items-center rounded-full bg-brand-primary-soft px-3 py-1.5 text-sm font-medium text-brand-primary">
                    ...
                    <button
                      ...
                      className="cursor-pointer mt-4 rounded-full bg-brand-primary px-5 py-2 text-sm font-semibold text-white transition hover:bg-brand-primary-hover"
                    >
